...
const calculatePremiumIndicators = (candles: Candle[]) => {
  ...
  return {
    ...
    tenkanSen: (Math.max(...highs.slice(-9)) + Math.min(...lows.slice(-9))) / 2,
    kijunSen: (Math.max(...highs.slice(-26)) + Math.min(...lows.slice(-26))) / 2,
    stochK: ((last.close - Math.min(...lows.slice(-14))) / (Math.max(...highs.slice(-14)) - Math.min(...lows.slice(-14)))) * 100,
...
const App: React.FC = () => {
  ...
  const [pendingSignal, setPendingSignal] = useState<Signal | null>(null); // ০ সেকেন্ডের জন্য জমা থাকবে
  ...
  // --- স্মার্ট টাইমার লজিক (০০ সেকেন্ডে সিগন্যাল পাবলিশ) ---
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const displayRemaining = remaining === 60 ? 0 : remaining;
      setCandleCountdown(displayRemaining);

      // যখনই ঘড়িতে ০০ সেকেন্ড বাজবে
      if (lastSecondRef.current !== seconds && displayRemaining === 0) {
        if (pendingSignal) {
          setCurrentSignal(pendingSignal); // জমা থাকা সিগন্যাল স্ক্রিনে দেখাবে
  ...
  const triggerAICall = useCallback(async () => {
    ...
    try {
      ...
      setPendingSignal(result); // রেজাল্ট পেয়ে গেলে জমা করে রাখবে
    } catch (e) {
      setIsAnalyzing(false);
    }
  }, [candles, selectedAsset]);

  // বাটন ক্লিকের হ্যান্ডলার (১০ সেকেন্ড আগে চাপলে কাজ করবে)
  const handleGetSignalClick = () => {
    if (isAnalyzing || candleCountdown > 12) return; // ১২-১০ সেকেন্ডের বেশি থাকলে কাজ করবে না
    
    setIsAnalyzing(true);
    setCurrentSignal(null);
    setPendingSignal(null);

    // ৪ সেকেন্ড ডিলে দিয়ে কল পাঠাবে (যাতে ঠিক ০০ সেকেন্ডের আগেই ডাটা রেডি হয়)
    setTimeout(() => {
      triggerAICall();
    }, 4000);
  };
  ...
  useEffect(() => {
    ...
    ws.current = socket;
    socket.onopen = () => {
      socket.send(JSON.stringify({ ticks_history: selectedAsset.id, count: HISTORY_COUNT, granularity: 60, style: "candles", subscribe: 1 }));
    };
  ...
  if (hasKey === false) {
    return (
      <div className="min-h-screen bg-[#0b0e11] flex flex-col items-center justify-center p-8 text-center">
        <h1 className="text-2xl font-black mb-4 text-white uppercase tracking-wider">SAKIB AI SIGNAL</h1>
        <button onClick={handleOpenKey} className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-bold">Select API Key</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0b0e11] text-white p-3 font-sans select-none overflow-x-hidden pb-10">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        ...
        <div className="bg-white/5 p-2.5 rounded-xl border border-white/5 flex flex-col items-center">
          <span className="text-[8px] text-gray-500 uppercase font-black">Win</span>
      ...
      <div className="flex justify-between items-center mb-4 bg-white/5 p-4 rounded-2xl border border-white/10">
        <div>
          ...
          <p className="text-2xl font-black text-white font-mono mt-1">{marketTime}</p>
        </div>
        <div className="text-right">
          <span className="text-gray-500 text-[9px] font-black uppercase">Next Candle</span>
      ...
      <div className="grid grid-cols-4 gap-2 mb-4 overflow-y-auto max-h-[100px] no-scrollbar">
        {ASSETS.map(asset => (
          <button key={asset.id} onClick={() => { setSelectedAsset(asset); setCandles([]); setCurrentSignal(null); }} className={`py-3 px-2 rounded-xl text-[9px] font-black border ${selectedAsset.id === asset.id ? 'bg-indigo-600 border-indigo-400' : 'bg-[#1c2127] border-white/5 text-gray-500'}`}>
      ...
      <div className="mt-6">
        <button 
          onClick={handleGetSignalClick} 
          disabled={isAnalyzing || candleCountdown > 12} 
          className={`w-full py-6 rounded-2xl font-black text-xl uppercase transition-all shadow-xl border-b-8 flex flex-col items-center justify-center ${
            isAnalyzing ? 'bg-amber-600 border-amber-800 animate-pulse cursor-wait' : 
            candleCountdown <= 12 ? 'bg-indigo-600 border-indigo-800 active:translate-y-1 active:border-b-4' : 
            'bg-gray-800 border-gray-900 opacity-50 cursor-not-allowed'
          }`}
        >
          {isAnalyzing ? "WAITING FOR 00s..." : (candleCountdown <= 12 ? "GET SURE SHOT" : "WAIT FOR 10s")}
        </button>
        <p className="text-[10px] text-center mt-2 text-gray-500 font-bold uppercase tracking-widest">
          {candleCountdown <= 12 ? "এখন ক্লিক করুন!" : "১০ সেকেন্ড বাকি থাকতে ক্লিক করুন"}
        </p>
      </div>
